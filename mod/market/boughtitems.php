...
$list_query = "SELECT TOP ".$lines." * FROM [DTweb_Market] where id Not in (Select TOP {$limit} id FROM [DTweb_Market] order by start_date desc) and [is_sold]='1' and [seller]!='".$user."' and [purchased_by] ='".$user."' order by start_date desc";
$sql = mssql_query("SELECT count(*) FROM [DTweb_Market] where is_sold='0' and seller!='".$user."' and [purchased_by] = '".$user."'");
$items_in_market = mssql_num_rows($sql);
$total = $items_in_market/$settings['pagination'];
$sql         = mssql_query($list_query);

$message[] = "    
       ".pagi_style("market&select=bought",$total,$settings['pagination'])." 
...
$chk_prices    = mssql_fetch_array(mssql_query("Select * from DTweb_Market where purchased_by='".$user."' and seller!='".$user."' and is_sold='1'")); 
...
$message[] = "
    <tr style='border:3px solid rgba(64,16,0,0.8);padding:20px 20px;background-color : ".$bgcolor."'>
          ...
          ".phrase_bought_date." <b>" . date("d/m/Y g:i a", $MarketItem['sold_date']) . "</b></td>
