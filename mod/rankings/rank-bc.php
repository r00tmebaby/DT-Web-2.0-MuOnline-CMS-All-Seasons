...
$page  = (int)($pag);
   ...
   if($set[12] > 0 ){
        $selecta = $set[12] * $set[13];
         $count = mssql_num_rows(mssql_query("Select TOP ". $selecta ." * from EVENT_INFO_BC where Point > 0"));	   
   }
   else{
         $count = mssql_num_rows(mssql_query("Select * from EVENT_INFO_BC where Point > 0 "));	
   }
   
   $max_pages  = ceil($count/$set[13]);
   $offset     = ($set[13]) * ($page - 1); 
   $select     = mssql_query(pagination($offset, $set[13], '[CharacterName],[PlayCount],[Point]', 'EVENT_INFO_BC', ' Point desc' ,'CharacterName','Point > 0'));  
   pagi_style("topbc",$max_pages,$set[13]);
...
<table class='table'>
	...
	<tr class="title">
		...
		<td><?php echo phrase_top_bc_played?></td>
